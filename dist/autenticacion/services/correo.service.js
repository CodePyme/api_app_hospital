...
let CorreoService = CorreoService_1 = class CorreoService {
    configService;
    logger = new common_1.Logger(CorreoService_1.name);
    transporter = null;
    ...
    inicializarTransporter() {
        ...
        const encryption = this.configService.get('MAIL_ENCRYPTION') ||
            (this.configService.get('SMTP_SECURE') === 'true' ? 'ssl' : 'tls');
        ...
        if (user && pass && host) {
            ...
            this.logger.warn('Servicio de correo iniciado en modo desarrollo (sin credenciales SMTP). Los códigos OTP se imprimirán en consola.');
        }
    }
    async enviarCodigoOtp(parametros) {
        const { destinatario, nombrePaciente, codigoOtp, minutosValidez = 10, nombreEntidad = 'Portal Paciente', logoUrl = '', colorPrimario = '#075c39', colorSecundario = '#9cc516', } = parametros;
        ...
        this.logger.log(`🔑 [OTP GENERADO] Para: ${destinatarioFinal} (Original: ${destinatario}) | Paciente: ${nombrePaciente} | Código: ${codigoOtp} | Tenant: ${nombreEntidad}`);
        if (!this.transporter) {
            this.logger.log(`✉️ [MODO SIMULADO] Correo simulado exitosamente a ${destinatarioFinal} con código OTP: ${codigoOtp}`);
            return true;
        }
        try {
            ...
            this.logger.error(`❌ Error al enviar correo OTP a ${destinatarioFinal}: ${error.message}`, error.stack);
            return false;
        }
    }
    generarPlantillaOtp(datos) {
...
exports.CorreoService = CorreoService = CorreoService_1 = __decorate([

...
export class ResultadosService {
  ...
  async obtenerDetalleOrden(internalNumber: string, clientCode = '') {
    ...
    const codeReporte = this.configService.get<string>('LABCORE_REPORT_CODE') || 'LRPT001';
    ...
    try {
      ...
      if (!resp.ok) {
        ...
        throw new BadRequestException(`Error al consultar el detalle de la orden (HTTP ${resp.status})`);
      }
